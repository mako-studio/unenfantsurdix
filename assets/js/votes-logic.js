...
(function() {
  ...
  function fingerprint(ref) {
    return scrutinUids.map(uid => {
      ...
      const s = scrutins[uid];
      const label = pos ? posLabel[pos] : 'Pas encore élu·e ou non recensé';
      return '<span class="vote-pill ' + (pos || 'N') + '" title="' + escAttr(s.date + ' — ' + s.titre + ' — ' + label) + '"></span>';
    }).join('');
  }
  ...
  function detailRows(ref) {
    return scrutinUids.map(uid => {
      ...
      const s = scrutins[uid];
      const label = pos ? posLabel[pos] : 'n/d';
      return '<div class="vote-card-detail-row"><span>' + s.date + ' — ' + s.titre + '</span><span class="vote-card-detail-pos ' + (pos || '') + '">' + label + '</span></div>';
    }).join('');
  }
  ...
  function renderChart(uid) {
    ...
    const groupCounts = {};
    Object.keys(deputes).forEach(ref => {
      const d = deputes[ref];
      const pos = votes[uid][ref];
      if (!pos) return;
      groupCounts[d.groupe] = groupCounts[d.groupe] || { P: 0, C: 0, A: 0, N: 0 };
      groupCounts[d.groupe][pos]++;
    });
    ...
    chartEl.innerHTML = order.map(code => {
      ...
      return '<div class="gbar-item">' +
        ...
        '<div class="gbar-item-count">' + c.P + ' pour · ' + c.C + ' contre · ' + c.A + ' abst. · ' + c.N + ' absent·e</div>' +
        '</div>';
    }).join('');
  }
